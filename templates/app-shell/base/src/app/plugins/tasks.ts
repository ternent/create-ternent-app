...
export function selectTaskById(state: TasksState, taskId: string): TaskRecord | null {
  ...
  return {
    ...
    commands: {
      "task.create": async (_ctx, input: TaskRecord) => {
        const protection = privacy.resolveAudience(
          {
      ...
      "task.rename": async (_ctx, input: { taskId: string; title: string }) => ({
        kind: "task.rename",
        payload: input,
      }),
    },
    applyEntry(entry, ctx) {
      ...
      if (entry.kind === "task.rename" && entry.payload.type === "plain") {
        const payload = entry.payload.data as { taskId: string; title: string };
        const current = state.byId[payload.taskId];
        if (!current) {
          return;
        }
